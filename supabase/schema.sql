...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  phone        VARCHAR(20),
  jersey_no    SMALLINT,          -- 如果该用户本身也是球员，记录球衣号
...
CREATE TABLE IF NOT EXISTS players (
  ...
  avatar_url   TEXT,
  phone        VARCHAR(20),
  user_id      UUID REFERENCES profiles(id) ON DELETE SET NULL,   -- 关联系统账号（可选）
  is_active    BOOLEAN NOT NULL DEFAULT TRUE,
  notes        TEXT,
...
CREATE TABLE IF NOT EXISTS teams (
  ...
  logo_url     TEXT,
  color        VARCHAR(7) DEFAULT '#1565c0',   -- 主题色 hex
  description  TEXT,
  is_active    BOOLEAN NOT NULL DEFAULT TRUE,
  created_by   UUID REFERENCES profiles(id),
  created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS games (
  ...
  game_type       VARCHAR(20) NOT NULL DEFAULT 'entertainment'
                  ...
                  -- entertainment: 120分娱乐制; official: 正式标准制
  ...
  quarter_seconds SMALLINT DEFAULT 600,   -- 每节10分钟
  current_quarter SMALLINT DEFAULT 1,
  quarter_clock   SMALLINT DEFAULT 600,   -- 当前节剩余秒数
  venue           VARCHAR(200),
...
-- ============================================================
-- 7. 上场阵容表 game_lineup（记录当前/历史上场5人）
-- ============================================================
CREATE TABLE IF NOT EXISTS game_lineup (
  ...
  slot_no     SMALLINT NOT NULL CHECK (slot_no BETWEEN 1 AND 5),  -- 面板位置1-5
...
CREATE TABLE IF NOT EXISTS game_stats (
  ...
  game_type       VARCHAR(20) NOT NULL,   -- 冗余存储，便于按赛制筛选
  -- 得分
  pts             SMALLINT NOT NULL DEFAULT 0,
  fgm             SMALLINT NOT NULL DEFAULT 0,   -- 投篮命中
  fga             SMALLINT NOT NULL DEFAULT 0,   -- 投篮出手
  fg3m            SMALLINT NOT NULL DEFAULT 0,   -- 三分命中
  fg3a            SMALLINT NOT NULL DEFAULT 0,   -- 三分出手
  ftm             SMALLINT NOT NULL DEFAULT 0,   -- 罚球命中
  fta             SMALLINT NOT NULL DEFAULT 0,   -- 罚球出手
  -- 其他数据
  reb             SMALLINT NOT NULL DEFAULT 0,   -- 篮板
  oreb            SMALLINT NOT NULL DEFAULT 0,   -- 进攻篮板
  dreb            SMALLINT NOT NULL DEFAULT 0,   -- 防守篮板
  ast             SMALLINT NOT NULL DEFAULT 0,   -- 助攻
  stl             SMALLINT NOT NULL DEFAULT 0,   -- 抢断
  blk             SMALLINT NOT NULL DEFAULT 0,   -- 盖帽
  tov             SMALLINT NOT NULL DEFAULT 0,   -- 失误
  pf              SMALLINT NOT NULL DEFAULT 0,   -- 犯规
  min_played      SMALLINT DEFAULT 0,            -- 上场分钟数
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS action_logs (
  ...
  -- action_type: 'pts_1','pts_2','pts_3','reb','oreb','dreb','ast','stl','blk','tov','pf',
  --              'fga_miss','fg3a_miss','fta_miss',
  --              'lineup_on','lineup_off','quarter_start','quarter_end','game_end'
  delta       SMALLINT NOT NULL DEFAULT 1,    -- +1 普通录入; -1 撤销/修正
  quarter     SMALLINT,
  game_clock  VARCHAR(10),                    -- 如 "08:23"
  recorded_by UUID NOT NULL REFERENCES profiles(id),
  is_voided   BOOLEAN NOT NULL DEFAULT FALSE,  -- 是否已撤销
...
CREATE TABLE IF NOT EXISTS game_mvp (
  ...
  -- 公式: pts*1.2 + reb*1.1 + ast*1.5 + stl*2 + blk*2 - tov*1.5 - pf*0.8
